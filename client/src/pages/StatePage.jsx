    ...
    return (
        ...
        <div className="bg-cover bg-center bg-no-repeat h-screen overflow-hidden" style={{backgroundImage: `url(${currState.bgImg})`}}>
            <div className="font-space">
                ...
                <div className="text-black animate-pulse font-semibold text-sm right-10 top-10 absolute cursor-pointer transition-all ease-in-out duration-300 gap-1 hover:text-white flex items-center">
                    <MoveLeft className="w-5"/>
                    <Link to='/map'>Back to map</Link>
                </div>

                <div className="border border-gray-200/20 border-t-gray-100 border-b-0 bg-gray-500/30 rounded-xl m-12 mb-15 p-6 w-1/3">
                    <h1 className="text-4xl font-bold mb-10">{currState.name}</h1>
                    <p>{currState.brief}</p>
                </div>

                <div className="flex items-center justify-center gap-8">
                    <div className="flex flex-col items-center gap-2 border border-gray-300/20 rounded-xl bg-gray-600/30 p-5 w-60">
                        <Landmark />
                        <h1 className="text-gray-300">Capital</h1>
                        <p className="font-semibold text-lg">{currState.capital}</p>
                    </div>

                    <div className="flex flex-col items-center gap-2 border border-gray-300/20 rounded-xl bg-gray-600/30 p-5 w-50">
                       <Users />
